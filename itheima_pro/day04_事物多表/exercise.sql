...
/*
	员工信息。查询员工编号，员工姓名，工资-> 需要查询emp表	职务名称，职务描述-> 需要查询job表
...
/*
		分析
			员工信息。查询员工编号，员工姓名，工资-> 需要查询emp表		职务名称，职务描述-> 需要查询job表 		部门名称,部门位置dept表
...
/*
	分析:	
		员工姓名，工资-> emp	工资等级 ->查询salarygrade
...
/*
	分析:
		员工姓名，工资->emp		职务名称，职务描述，job		部门名称，部门位置，dept	工资等级salarygrade
...
/*
	分析:
		部门编号、部门名称、部门位置dept表		部门人数emp表
        使用分组查询,按照emp.dept_id完成分组,查询count(id)
        使用子查询将第二步的查询结果和dept表进行关联查询
*/
...
/*
	分析:	
		姓名 emp		直接上级的姓名 emp
			emp表的id	和	mgr是自关联的
		条件	emp.id = emp.mgr
        没有领导的员工也需要查询:查询左表的所有数据,和 交集数据->使用左外连接查询
